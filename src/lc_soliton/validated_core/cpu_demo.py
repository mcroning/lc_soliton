...
def _compute_h_local(ctx, dz_um: float):
...
def _hop_linear_local(ctx, amp, h):
    xp = ctx.xp
...
def _lc_dn_local(theta, ctx):
    return (compute_neff(theta, ne=ctx.p.ne, no=ctx.p.no, xp=ctx.xp) - ctx.refin).astype(ctx.xp.float32)


def _propagate_slice_local(ctx, amp, theta, *, nsub: int, dz_sub: float, h_sub):
    xp = ctx.xp
...
def _residual_local(theta, I, ctx):
    p = ctx.p
    xp = ctx.xp
...
def _simple_static_smoke_slice(ctx, amp, theta_seed, *, nsub: int, dz_sub: float, h_sub):
