#!/usr/bin/env python3
"""One-z-pass TD orchestration equivalence against trusted runner_core.

This validates that the newly extracted engines reproduce one complete
production TD pass over z:
    ...
    for k in z:
        I_b = intensity(amp)
        I_mid = advance optics through slice k using theta_ref[k]
        theta_full[k] = td_update_theta_from_midintensity(... I_mid ...)
    half linear hop

The test deliberately compares orchestration, not just isolated kernels.
"""
...
def main() -> None:
    ...
    new_amp, new_theta, new_I_mid = _run_new_one_zpass(
        new_opt, new_th, ctx_new, amp0, theta0,
    ...
    print("47_td_one_zpass_runner_core_equivalence")
    ...
    for k, v in metrics.items():
        print(f"  {k:<22}: {v:.16e}" if isinstance(v, float) else f"  {k:<22}: {v}")

    failures = {k: v for k, v in metrics.items() if (k.endswith("rel_l2") or k.endswith("max_abs")) and float(v) > args.tol}
    if failures:
        raise AssertionError(f"one-z-pass differs from runner_core beyond tol={args.tol}: {failures}")
    print("47_td_one_zpass_runner_core_equivalence: PASS")
